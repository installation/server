...
preload() {
	echo -e "\033[34m###### Installing/Uninstalling Contacts plugin ######\033[0m"
...
install() {
	...
	sudo -u $user -H wget http://redminecrm.com/license_manager/4150/redmine_contacts-3_1_2-light.zip
	sudo -u $user -H unzip redmine_contacts-3_1_2-light.zip
	sudo bundle install --without development test sqlite postgresql
	sudo -u $user -H bundle exec rake redmine:plugins NAME=redmine_contacts RAILS_ENV=production
	sudo rm -rf redmine_contacts-3_1_2-light.zip
	echo -e "\033[34m###### Installation done ######\033[0m"
}

uninstall() {
	echo -e "\033[34m###### Uninstalling plugin ######\033[0m"
	sudo -u $user -H bundle exec rake redmine:plugins:migrate NAME=redmine_contacts VERSION=0 RAILS_ENV=production
	sudo -u $user -H rm -rf redmine_contacts/
