...
preload() {
	echo -e "\033[34m###### Installing/Uninstalling Finance plugin ######\033[0m"
...
install() {
	...
	sudo -u $user -H wget http://redminecrm.com/license_manager/4479/redmine_finance-1_0_1-light.zip
	sudo -u $user -H unzip redmine_finance-1_0_1-light.zip
	...
	sudo rm -rf redmine_finance-1_0_1-light.zip
	echo -e "\033[34m###### Installation done ######\033[0m"
}

uninstall() {
	echo -e "\033[34m###### Uninstalling plugin ######\033[0m"
	sudo -u $user -H bundle exec rake redmine:plugins:migrate NAME=redmine_finance VERSION=0 RAILS_ENV=production
	sudo -u $user -H rm -rf redmine_finance/
