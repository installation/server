...
preload() {
	echo -e "\033[34m###### Installing/Uninstalling Projects Table plugin ######\033[0m"
...
install() {
	...
	sudo -u $user -H git clone git://github.com/denispeplin/redmine-projects-table.git projects_table
	#sudo bundle install --without development test sqlite postgresql
	sudo -u $user -H bundle exec rake redmine:plugins NAME=projects_table RAILS_ENV=production
	echo -e "\033[34m###### Installation done ######\033[0m"
}

uninstall() {
	echo -e "\033[34m###### Uninstalling plugin ######\033[0m"
	sudo -u $user -H bundle exec rake redmine:plugins:migrate NAME=projects_table VERSION=0 RAILS_ENV=production
	sudo -u $user -H rm -rf projects_table/
